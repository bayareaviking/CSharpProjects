...
namespace Pluralsight.ArraysCollections.Demos
{
    public class Bus
    {
        ...
        public void ArriveAt(string place)
        {
            ...
            List<Passenger> passengersToRemove = _passengers.FindAll(x => x.Destination == place); // Identifying all passengers getting off and storing them in a list
            _passengers.RemoveAll(x => x.Destination == place); // Separate action to remove all passengers getting off from _passengers

            foreach (Passenger passenger in passengersToRemove) // Iterating new list to display passengers getting off
